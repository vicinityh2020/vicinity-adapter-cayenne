...
class TestCayenneParser(unittest.TestCase):
    ...
    def test_Digital_Out (self):
        ...
        cayenne = cayenne_parser.CayenneParser()        
        out = cayenne.decodeCayenneLpp(b64, datetime.datetime.now())                     
        hit = py_.find (out, {"id": "DigitalOut"})        
        self.assertTrue(hit)
        self.assertTrue(int(hit["value"]) == 1)
    ...
    def test_Illuminance (self):
        ...
        cayenne = cayenne_parser.CayenneParser()        
        out = cayenne.decodeCayenneLpp(b64, datetime.datetime.now())                    
        hit = py_.find (out, {"id": "Illuminance"})             
        self.assertTrue(hit)
    ...
    def test_Temperature (self):
        ...
        cayenne = cayenne_parser.CayenneParser()        
        out = cayenne.decodeCayenneLpp(b64, datetime.datetime.now())                    
        hit = py_.find (out, {"id": "Temperature"})        
        self.assertTrue(hit)
    ...
    def test_RelativeHumidity (self):
        ...
        cayenne = cayenne_parser.CayenneParser()        
        out = cayenne.decodeCayenneLpp(b64, datetime.datetime.now())                    
        hit = py_.find (out, {"id": "RelativeHumidity"})        
        self.assertTrue(hit)
    ...
    def test_Accelerometer (self):
        ...
        hit = py_.find (out, {"id": "AccelerometerX"})        
        self.assertTrue(hit)
        self.assertAlmostEqual(float(hit["value"]), 1.234, delta=0.1)

        hit = py_.find (out, {"id": "AccelerometerY"})        
        self.assertTrue(hit)
        self.assertAlmostEqual(float(hit["value"]), -1.234, delta=0.1)

        hit = py_.find (out, {"id": "AccelerometerZ"})        
        self.assertTrue(hit)
    ...
    def test_Barometer (self):
        ...
        cayenne = cayenne_parser.CayenneParser()        
        out = cayenne.decodeCayenneLpp(b64, datetime.datetime.now())                     
        hit = py_.find (out, {"id": "BarometricPressure"})                
    ...
    def test_Gyrometer (self):
        ...
        hit = py_.find (out, {"id": "GyrometerX"})        
        self.assertTrue(hit)
        self.assertAlmostEqual(float(hit["value"]), 12.34, delta=0.1)

        hit = py_.find (out, {"id": "GyrometerY"})        
        self.assertTrue(hit)
        self.assertAlmostEqual(float(hit["value"]), -12.34, delta=0.1)

        hit = py_.find (out, {"id": "GyrometerZ"})        
        self.assertTrue(hit)
        self.assertAlmostEqual(float(hit["value"]), 0.0, delta=0.1)
